	...
	return {
		...
		handlers: function(instaElements) {
			...
			function applyOffset(selector){
				...
				if (window.innerWidth > 768) {
					setTimeout(function(){
						...
						$(selector).css("padding-top", head_height - 1 + 'px');
					},210);
				}else{
				...
				window.addEventListener("resize", function() {
					...
					const head_height = $(".js-head-main").height();
					...
					if (window.innerWidth > 768) {
						...
						// On mobile there is no top padding due to position sticky CSS
						jQueryNuvem(selector).css("padding-top", "0px");
					}
				});
			}
			...
			function updateHeaderOffset() {
				const headFixedDesktop = $(".js-head-main").attr("data-header-md-fixed");
				if(headFixedDesktop == 'true'){
			...
			handlers.head_fix_desktop = new instaElements.Lambda(function(headFix){
				...
				if (window.innerWidth > 768) {
					if (headFix) {
						$head.addClass('position-fixed-md').removeClass("position-relative-md");
						var head_height = $(".js-head-main").height();
						$body.css("padding-top", head_height); 
						$head.attr("data-header-md-fixed", "true");
					} else {
						$head.removeClass('position-fixed-md').addClass("position-relative-md");
