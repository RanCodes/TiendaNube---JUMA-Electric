...
DOMContentLoaded.addEventOrExecute(() => {
    ...
    jQueryNuvem(document).on("click", ".js-addtocart:not(.js-addtocart-placeholder)", function (e) {
        ...
        const initialButtonText = $productButton.val();
        ...
        if (!jQueryNuvem(this).hasClass('contact')) {
            ...
            {# Disable button and show loading text during event #}

            $productButton.prop('disabled', true);
            $productButton.val("{{ 'Agregando a carrito...' | translate }}");
            ...
            {% if settings.ajax_cart %}
                ...
                var callback_add_to_cart = function(html_notification_related_products, html_notification_cross_selling) {
                    ...
                    $productButton.val("{{ '¡Listo!' | translate }}");
                    setTimeout(function(){
                        $productButton.val(initialButtonText).prop('disabled', false);
                    },2000);
                ...
                var callback_error = function(){
                    {# Restore real button text in case of error #}
    ...
    // Assign custom color classes to shipping options based on their names
    applyShippingColorClasses = function() {
        jQueryNuvem('.js-shipping-radio').each(function() {
            var name = jQueryNuvem(this).find('input[type="radio"]').data('name') || '';
            var lower = name.toLowerCase();
            if (lower.indexOf('correo argentino') !== -1) {
                jQueryNuvem(this).addClass('shipping-correo-argentino');
            } else if (lower.indexOf('andreani') !== -1) {
                jQueryNuvem(this).addClass('shipping-andreani');
            } else if (lower.indexOf('retiro por juma') !== -1) {
                jQueryNuvem(this).addClass('shipping-retiro-juma');
            } else if (lower.indexOf('retiro') !== -1) {
                jQueryNuvem(this).addClass('shipping-retiro');
            }
            ...
            var slug = lower
                ...
                .replace(/^-+|-+$/g, '');
            if (slug) {
                jQueryNuvem(this).addClass('shipping-' + slug);
            }
        });
    };
